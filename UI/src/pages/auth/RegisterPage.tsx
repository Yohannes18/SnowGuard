...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!token) {
        // some backends return user object only; attempt to login using credentials
        const maybeLogin = await import("../../services/auth").then(m => m.loginUser);
        const loginRes = await maybeLogin({ username: username || email, password });
        const loginToken = (loginRes as any).access_token || (loginRes as any).token;
        if (!loginToken) throw new Error("Registration succeeded, but auto-login failed.");
